...
Set each real SSH address once in `inventory/hosts.ini`. The name before `ansible_host` is Ansible's stable alias and becomes the RKE2 node name; it does not need to resolve in DNS. Port 22 is the default. Add `ansible_port=<port>` only when SSH uses a different port:

```ini
[rke2_servers]
rke2-server-1.airgap.local ansible_host=<server1_IP>
rke2-server-2.airgap.local ansible_host=<server2_IP> ansible_port=2222
```

The matching files under `host_vars/` contain only node-specific RKE2 labels and taints, not duplicate connection IPs. Also review `group_vars/all.yml` for cluster-wide settings.
