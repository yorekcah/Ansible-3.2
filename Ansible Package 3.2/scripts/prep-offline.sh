...
K8SGPT_VERSION="${K8SGPT_VERSION:-v0.3.40}"
...
VELERO_VERSION="${VELERO_VERSION:-v1.18.2}"
VELERO_PLUGIN_VERSION="${VELERO_PLUGIN_VERSION:-v1.14.2}"
...
fetch "https://github.com/k8sgpt-ai/k8sgpt/releases/download/${K8SGPT_VERSION}/k8sgpt_Linux_x86_64.tar.gz" "$DEST/artifacts/k8sgpt.tar.gz"
...
pull_chart vmware-tanzu velero "$VELERO_CHART_VERSION"
pull_chart istio base "$ISTIO_VERSION"
pull_chart istio istiod "$ISTIO_VERSION"
pull_chart istio gateway "$ISTIO_VERSION"
...
valkey:
  ...
  primary:
    persistence: {size: 5Gi, storageClass: longhorn}
EOF2
cat > "$WORK/values/velero.yaml" <<EOF2
configuration:
  provider: aws
credentials:
  useSecret: false
initContainers:
  - name: velero-plugin-for-aws
    image: docker.io/velero/velero-plugin-for-aws:${VELERO_PLUGIN_VERSION}
    volumeMounts:
      - mountPath: /target
        name: plugins
image:
...
render velero "$WORK/values/velero.yaml"
render base /dev/null
render istiod /dev/null
render gateway <(printf 'service:\n  type: LoadBalancer\n')
