...
At minimum, set the RKE2 token and Harbor/Gitea/Velero credentials in the vault.
...
10. Istio, Garage, Gitea, Velero, k9s and k8sgpt
...
The script does not read the RKE2 configuration, cluster token, or vault files. It reports RHEL/SELinux RPM state, RKE2 service status and recent logs, cluster nodes and pods, Traefik, services, ingresses, storage, Velero backup storage, and containerd images. Review the collected service logs before sharing them.
